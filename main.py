...
##############################################################################################################  --- Historical Analysis --- ###############################################################################################################
    ...
    initial_date = today - timedelta(days=13)
...
#S
    ...
    # df2 = pd.DataFrame({
    #     'Dates': all_dates_between_initial_final_dates,
    #     'Automated': [result[0] for result in results], #results2
    #     'Manual': [result[1] for result in results]  #results2
    # })

    # # Melt the dataframe to a long format for Altair
    # df_long2 = df2.melt('Dates', var_name='Type', value_name='DAILY Average Moderation Time [Minutes]')

    # # Create an Altair chart
    # chart = alt.Chart(df_long2).mark_line().encode(
    #     x='Dates',
    #     y='DAILY Average Moderation Time [Minutes]',
    #     color=alt.Color('Type', scale=alt.Scale(domain=['Automated', 'Manual'], range=['red', 'green'])),
    #     strokeDash='Type').properties(
    #     title='Average Moderation Time minutes')
    


    # ####################################################
    # #graph4
    # acc_total = df2['Automated'].mean()
    # user_total = df2["Manual"].mean()

    # # Create a DataFrame with the totals
    # data_a = {'Category': ['Automated', 'Manual'],'Moderation Time [Min]': [acc_total, user_total]}
    ...
    # chart_two = alt.Chart(df_xx).mark_bar().encode(
    #     x=alt.X('Category:N', title=''),
    #     y=alt.Y('Average Moderation Time:Q', title='Moderation Time [Min]'),
    #     color=alt.Color('Category:N', scale=color_scale, legend=None)).properties(width=alt.Step(80))
